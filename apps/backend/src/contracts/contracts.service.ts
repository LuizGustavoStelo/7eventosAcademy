...
export class ContractsService {
  ...
  async sendInstance(
      ...
      formsAndValuesSummaryLines.push(
        `Valor total com desconto${
          selectedOptionDiscountDeadlineDay > 0
            ? ` atÃ© dia ${selectedOptionDiscountDeadlineDay}`
            : ''
    ...
    if (selectedPaymentOption?.note) {
      formsAndValuesSummaryLines.push(`ObservaÃ§Ã£o: ${selectedPaymentOption.note}`);
    }
  ...
  async signInstitutionInstance(instanceId: string, actor: ContractActor) {
    ...
    if (
      ...
      throw new BadRequestException(
        'A assinatura institucional nÃ£o pode ser registrada para este status.',
      );
    }
  ...
  async verifyMyPin(instanceId: string, pin: string, actor: ContractActor) {
    ...
    if (!institutionSignature.signedAt) {
      throw new BadRequestException(
        'A instituiÃ§Ã£o precisa assinar o contrato antes da assinatura do aluno.',
      );
    }
    ...
    return Array.from({ length: months }).map((_, index) => {
      ...
      const discountLabel = hasDiscountInstallment
        ? `${this.formatCurrencyPtBr(discountValue)}${
            hasDeadline ? ` atÃ© dia ${discountDeadlineDay}` : ''
          }${crfLabel}`
        : null;
      const detailsLabel = hasDiscountInstallment
        ? `${hasDeadline ? `AtÃ© dia ${discountDeadlineDay}` : 'Pagamento antecipado'}: ${this.formatCurrencyPtBr(
            discountValue,
          )}. ${
            hasDeadline ? `ApÃ³s dia ${discountDeadlineDay}` : 'Valor regular'
    ...
    return `<table style="width:100%;border-collapse:collapse;font-family:Arial,sans-serif;font-size:12px;">
      <thead>
        <tr>
          ...
          ${
            hasDetails
              ? '<th style="text-align:left;border:1px solid #d1d5db;padding:6px;">ObservaÃ§Ãµes</th>'
              : ''
          }
        </tr>
      </thead>
